...
class _BookingScreenState extends State<BookingScreen> {
  ...
  Future<void> _fetchSchedule() async {
    try {
      ...
      // DEBUG: Revisa esto en tu terminal para ver si la API responde con datos
      print('DEBUG: Horarios del negocio cargados: $schedule');
      ...
      print('DEBUG: Error al obtener horarios: $e');
  ...
  List<String> _getAvailableTimes() {
    ...
    final newServiceDuration = Duration(
    ...
    while (current.isBefore(lastSlotPossible) ||
        current.isAtSameMomentAs(lastSlotPossible)) {
      // Validamos que la hora no haya pasado ya si es el día de hoy
      if (current.isAfter(DateTime.now())) {
        ...
        for (var apt in existingAppointments) {
          ...
          final aptEnd = aptStart.add(
            Duration(minutes: apt.service?.durationMinutes ?? 30),
          );
  ...
  Widget _buildCalendar(ColorScheme cs) {
    return TableCalendar(
      // Se modificó para permitir selección aunque la lista de horarios tarde en cargar
      enabledDayPredicate: (day) {
        if (_businessSchedules.isEmpty)
                ...
                child: Wrap(
                  ...
                  children: times.map((t) {
                    ...
                    return ChoiceChip(
                      ...
                      selected: isSel,
                      onSelected: (val) =>
                          setState(() => _selectedTime = val ? t : null),
    ...
    try {
      ...
      if (mounted) {
        navigator.pushAndRemoveUntil(
          MaterialPageRoute(
            builder: (_) =>
                const MainNavigation(userRole: 'CLIENT', initialIndex: 2),
          ),
          (route) => false,
        );
        navigator.push(
          MaterialPageRoute(
            builder: (_) => _BookingSuccessPage(appointment: aptWithRelations),
          ),
        );
      }
    } catch (e) {
...
class _BookingSuccessPageState extends State<_BookingSuccessPage>
    ...
    Future.delayed(const Duration(milliseconds: 2800), () {
      if (mounted) {
        Navigator.pushReplacement(
          ...
          PageRouteBuilder(
            pageBuilder: (_, _, _) =>
                AppointmentDetailScreen(appointment: widget.appointment),
                              ...
                              if (widget.appointment.business?.logoUrl != null)
                                CircleAvatar(
                                  ...
                                  backgroundImage: NetworkImage(
                                    widget.appointment.business!.logoUrl!,
                                  ),
